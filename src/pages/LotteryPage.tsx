...
import { Armchair, Check, Crown, DoorOpen, LoaderCircle, Sparkles, Ticket, Trash2, Users } from 'lucide-react';
...
import { cancelLotteryEntry, ensureDeviceToken, getPublicLotterySnapshot, LOTTERY_UPDATED_EVENT, submitLotteryEntry } from '../lib/lottery-api';

const stateLabels = { accepting: '応募受付中', drawing: '抽選中', drawn: '結果公開待ち', published: '結果発表', closed: '受付終了' };
...
export const LotteryPage = () => {
  ...
  const refresh = useCallback(async () => {
    try { setSnapshot(await getPublicLotterySnapshot(token)); setError(''); }
    catch (caught) { setError(caught instanceof Error ? caught.message : '通信に失敗しました'); }
  ...
  const submit = async () => {
    ...
    try { await submitLotteryEntry(parsed.data); await refresh(); }
    catch (caught) { setError(caught instanceof Error ? caught.message : '応募の送信に失敗しました'); }
  ...
  const cancel = async () => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : '応募のキャンセルに失敗しました'); }
    finally { setSubmitting(false); }
  };
  ...
  return (
    <main className="relative min-h-screen w-full overflow-hidden text-[var(--color-text)]">
      <video aria-hidden="true" autoPlay className="fixed inset-0 h-full w-full object-cover" loop muted playsInline src="https://d8j0ntlcm91z4.cloudfront.net/user_38xzZboKViGWJOttwIXH07lWA1P/hf_20260518_003132_8b7edcb6-c64d-4a52-a9ca-879942e122ad.mp4" />
      ...
      <section className="relative z-[1] mx-auto max-w-7xl px-5 pb-12 pt-[clamp(28px,6vw,64px)] sm:px-8">
        <motion.div animate={{ opacity: 1, y: 0 }} className="max-w-[590px] rounded-[32px] border border-white/45 bg-[rgba(242,242,238,0.68)] p-5 shadow-[0_24px_80px_rgba(25,40,55,0.13)] backdrop-blur-xl sm:p-8" initial={{ opacity: 0, y: 24 }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}>
          {loading ? <div className="grid min-h-72 place-items-center" role="status"><LoaderCircle className="animate-spin" size={32} /></div> : entry ? (
            <div className="space-y-6">
              ...
              <dl className="grid gap-3 rounded-3xl bg-white/50 p-5 text-sm"><div className="flex justify-between gap-4"><dt className="opacity-55">応募項目</dt><dd className="font-bold">{kindLabels[entry.kind]}</dd></div><div className="flex justify-between gap-4"><dt className="opacity-55">代表者X</dt><dd className="font-bold">{entry.representativeId}</dd></div><div className="flex justify-between gap-4"><dt className="opacity-55">代表者VRC名</dt><dd className="font-bold">{entry.representativeVrcName}</dd></div>{entry.companionVrcName ? <div className="flex justify-between gap-4"><dt className="opacity-55">同行者VRC名</dt><dd className="font-bold">{entry.companionVrcName}</dd></div> : null}</dl>
              {snapshot?.settings.state === 'accepting' ? <button className="flex w-full items-center justify-center gap-2 rounded-full border border-red-300 bg-white/50 px-5 py-3.5 text-sm font-semibold text-red-700 transition hover:bg-red-50 disabled:opacity-50" disabled={submitting} onClick={() => { setError(''); setShowCancelConfirm(true); }} type="button"><Trash2 size={17} />応募をキャンセル</button> : null}
            </div>
        ...
        {!entry && snapshot?.settings.state === 'accepting' ? <GuestCoinGallery /> : null}
      </section>
      <AnimatePresence>
        {showCancelConfirm ? (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            initial={{ opacity: 0 }}
            onClick={() => { if (!submitting) setShowCancelConfirm(false); }}
          >
            <motion.section
            ...
            >
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                ...
                <button className="rounded-full bg-red-600 px-4 py-3.5 text-sm font-semibold text-white disabled:opacity-50" disabled={submitting} onClick={() => void cancel()} type="button">{submitting ? '処理中...' : 'キャンセルを確定'}</button>
              </div>
            </motion.section>
          </motion.div>
        ) : null}
